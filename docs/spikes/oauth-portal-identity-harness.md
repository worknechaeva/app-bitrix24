...
`BITRIX24_OAUTH_SPIKE_SCOPE_HYPOTHESIS` — только server-side ожидание для текущего запуска campaign. Допустимы `user_brief` и `user`; пустое значение, несколько значений, `user_basic`, `basic`, `task`, `tasks`, `crm`, webhook и неизвестные значения отклоняются. Initial token response должен содержать выбранную hypothesis, иначе flow завершается безопасным `scope_hypothesis_mismatch`. Token endpoint фиксирован на `https://oauth.bitrix.info/oauth/token/`. Redirect URI обязан принадлежать configured app origin и указывать на callback route.

`user_brief` — первая проверяемая гипотеза, а не подтвержденный достаточный production scope. План live campaign:

1. В настройках local application выбрать право `user_brief`, установить hypothesis `user_brief` и проверить admission.
...
3. В настройках local application заменить право на `user`, изменить hypothesis на `user` и повторить campaign.
...
Token-response scopes нормализуются как set без дублей. Initial set должен содержать configured hypothesis; refresh set должен полностью совпасть с initial и по-прежнему содержать hypothesis. Дополнительные фактические значения вроде `app`, `basic` или других системных scopes пока не ограничиваются production allowlist и фиксируются в sanitized campaign log для последующего решения. Точная семантика возвращаемого Bitrix24 поля `scope` остается предметом live campaign.
...
Browser получает только `success/error`, совпадение member ID, canonical portal origin, admission status, `refreshVerified: true` при полном успехе и безопасный reason code. Scopes, expiry, access token, refresh token, authorization code, client secret, Bitrix user ID, provider payload и stack trace в response не выводятся. Sanitized server log может содержать scope hypothesis, `hypothesisMatched` и нормализованные имена фактических scopes без token metadata.
