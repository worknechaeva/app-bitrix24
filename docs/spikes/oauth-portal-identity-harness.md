...
- `GET /api/bitrix24/oauth/start` — создает одноразовый state и перенаправляет на один настроенный portal; authorization URL содержит только `client_id` и `state`.
- `GET /api/bitrix24/oauth/callback` — consumes state, проверяет initial exchange, немедленно выполняет refresh rotation, повторно проверяет portal identity и token metadata, получает фактические права приложения методом `scope`, затем проверяет active employee только через rotated access token.
...
Фактические права выбираются пользователем в интерфейсе создания или редактирования local application Bitrix24. Изменение environment само по себе не меняет разрешения приложения, а authorization URL намеренно не содержит `scope`.

`BITRIX24_OAUTH_SPIKE_TOKEN_SCOPE_HYPOTHESIS` — отдельное server-side ожидание OAuth token scope. Для harness допустимо только `app`: initial token response должен содержать `app`, а refresh должен вернуть точно тот же нормализованный набор token scopes. Несовпадение initial hypothesis завершается безопасным `scope_hypothesis_mismatch`, drift после refresh — `oauth_metadata_drift`.
...
Refresh выполняется немедленно в том же callback request. Его `member_id`, canonical portal origin, нормализованный token scope set, expiry metadata и доступный provider user ID сверяются с initial exchange. После успешной проверки REST-метод `scope` вызывается без `full=true` с rotated access token и refreshed `client_endpoint`; его строгий массив строк подтверждает отдельную permission hypothesis. Только после этого теми же rotated credentials вызывается `getCurrentUser`. При drift или permission mismatch обе token pair отбрасываются без admission, profile, session или credentials.

Token-response scopes нормализуются как set без дублей. Initial set должен содержать `app`; refresh set должен полностью совпасть с initial и по-прежнему содержать `app`. Token scope `app` не интерпретируется как разрешение на пользователей, задачи или группы. Фактические права local application проверяются только отдельным REST-ответом `scope`, который нормализуется, ограничивается по формату, длине и количеству и сравнивается case-sensitive с permission hypothesis.
