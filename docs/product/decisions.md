...
- **Последствия:** Правила ownership одинаковы в mock и live. Administrator архивирует и восстанавливает чужие записи только узкими RPC. Физического удаления нет. Несколько записей одного owner могут ссылаться на один `bitrix_entity_id`; unique только по нему запрещен. Каждое фактическое archive/restore фиксируется append-only audit event в одной транзакции. Live UI получает entity и active employee через Directory, а сервер повторно проверяет оба выбора перед сохранением. Перед будущим вызовом live Task client проверки повторяются.
...
- **Последствия:** Без необходимости не запрашиваются email, телефоны, дата рождения, адрес, фотография и другие персональные данные. Полный список использует `user.get`, запрос — `user.search`; оба результата финально фильтруются по active employee. Контракты подтверждены DEC-037.
- **Связанные QA-записи:** —
- **Заменяет:** —
...
## DEC-038 — Идемпотентное создание launcher project

- **Дата:** 2026-09-09
- **Статус:** Active
- **Контекст:** Ответ на сохранение может потеряться после выполнения PostgreSQL RPC, а ошибка последующего чтения списка не означает, что сама мутация завершилась неуспешно. Обычный повтор create без устойчивой identity создает дубликат.
- **Решение:** Каждая открытая create-форма получает UUID операции, сохраняет его в actor-bound черновике до подтвержденного ответа и повторно использует после неизвестного результата. PostgreSQL связывает ключ с portal и owner, повторно проверяет действующую app session и возвращает существующий проект только при совпадении пользовательских полей. Подтвержденная мутация и последующее чтение списка возвращаются как независимые результаты.
- **Последствия:** Одновременные и последовательные повторы одного create не создают дубликаты; другой owner может использовать такое же значение ключа без доступа к чужой записи; измененный payload отклоняется. Сбой обновления списка показывает подтвержденный success и отдельное действие чтения, а ошибка transport снимает pending и сохраняет форму для безопасного явного повтора.
- **Связанные QA-записи:** QA-023.
- **Заменяет:** уточняет DEC-025 без изменения ownership и Directory-проверок.
