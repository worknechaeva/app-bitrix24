...
Этот документ фиксирует действующее требуемое поведение Task Launcher и утвержденные границы Milestone 2. Это не хронология обсуждений. Синхронизация документации не означает, что описанная ниже production-интеграция уже реализована: фактический код по-прежнему содержит завершенный development-only mock первого milestone, а реализация Milestone 2 еще не начиналась.
...
Identity и Directory получают live-реализации в Milestone 2 после успешных соответствующих technical spikes. Это целевой scope, а не утверждение об уже выполненной реализации. `Bitrix24TaskClient` остается mock только в development/test, а production использует `DisabledBitrix24TaskClient` с fail-closed поведением.
