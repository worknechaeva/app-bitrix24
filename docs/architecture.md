...
- Интерфейс хранит выбранные `File` только в памяти страницы и перед mock Server Action преобразует их в метаданные.
- Серверная mock-абстракция повторно валидирует метаданные и сохраняет только имя, MIME-тип и размер.
- Бинарные данные не попадают в sanitized payload, историю операций и логи.
- Бинарные данные в текущем mock-milestone не передаются в Server Action, поэтому размер выбранного файла не упирается в лимит тела action.
